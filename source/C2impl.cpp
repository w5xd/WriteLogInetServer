...
#include "C2BasicHttpBinding_USCOREIContestQsosObject.h"
#include "C2.nsmap"
...
int __contest21__GetSessionId(soap *s, _contest23__GetSessionId *p1, _contest23__GetSessionIdResponse *r)
{
    if (!r) return SOAP_NO_DATA;
...
int __contest21__addAndGetQsos(soap *s, 
    _contest23__addAndGetQsos *p1, _contest23__addAndGetQsosResponse *r)
{
    if (!p1 || !r) return SOAP_NO_DATA;
...
int __contest21__AddAndGetLogSummary(soap *s, _contest23__AddAndGetLogSummary *p1, _contest23__AddAndGetLogSummaryResponse *r)
{
    if (!p1 || !r) return SOAP_NO_DATA;
    if (!appState(s)->SessionOK(p1->SessionId))
        return soap_receiver_fault(s, "Invalid SessionId", "The Session ID must match the one sent at initialization time");
...
int __contest21__getQsosByKeyArray(soap *s, 
...
int __contest21__ColumnNamesToIndices(soap *s, 
...
int __contest21__ExchangeFrequencies(soap *s, 
